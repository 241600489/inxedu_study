...
public class AdminCourseController extends BaseController {
    ...
    @Autowired
    private CourseBaseService eduCourseBaseService;
    ...
    public ModelAndView showCourseList(HttpServletRequest request, @ModelAttribute("page") PageEntity page, @ModelAttribute("queryCourse") QueryCourse queryCourse) {
        ...
        try {
            ...
            Integer id = teacherService.queryTeacherIdByName(queryCourse.getTeacherName());
            if (ObjectUtils.isNotNull(id)) {
                queryCourse.setTeacherId(id);
            }
    ...
    public ModelAndView toAddCourse(HttpServletRequest request) {
        ...
        try {
            ...
            List<String> courseNOlist = eduCourseBaseService.queryCourseNO();
            model.addObject("courseNOlist", courseNOlist);
    ...
    public ModelAndView addCourse(HttpServletRequest request, @ModelAttribute("course") Course course) {
        ...
        try {
            ...
            course.setResidaulCount(course.getMaxNumber());
            //course.setIsavaliable(1);//上架
            CourseBase courseBase = courseBaseService.queryCourseNo(course.getCourseName());
            if (Objects.isNull(courseBase)) {

            }
            course.setCourseNo(courseBase.getCourseNo());
    ...
    public ModelAndView initUpdatePage(HttpServletRequest request, @PathVariable("courseId") int courseId) {
        ...
        try {
            ...
            //查询课程所属老师
            List<Map<String, Object>> teacherList = teacherService.queryCourseTeacerList(course.getTeacherId());
            List<Map<String, Object>> list1 = new ArrayList<>();
            for (Map it : teacherList
                    ) {
                String s = string2Json(it.get("career").toString());
                it.put("career", s);
                list1.add(it);
            }
            String s = gson.toJson(list1);
            model.addObject("teacherList", s);
        } catch (Exception e) {
            model.setViewName(this.setExceptionRequest(request, e));
