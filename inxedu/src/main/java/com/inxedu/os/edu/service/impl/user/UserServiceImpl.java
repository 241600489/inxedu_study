...
public class UserServiceImpl extends GenericDaoImpl implements UserService{
	...
	@Override
	public HyberUser getHyberUser(int userId) {
		User user=userDao.queryUserById(userId);
		List<User> list=new ArrayList<>();
		list.add(user);
		List<HyberUser>  list1=hyperUser(list);
	...
	public List<HyberUser> queryUserListPage(QueryUser query, PageEntity page) {
		   List<User> list=userDao.queryUserListPage(query, page);
		return hyperUser(list);
	}
	...
	public void updateClassIdByUserId(int userId,int classId){
		userDao.updateClassIdByUserId(userId, classId);
	}

	@Override
	public List<HyberUser> queryUserByNo(String no,PageEntity page) {
       User user=new User();
		user.setNo(no);
		List<User> list= this.queryForListPage("UserMapper.queryUserByNo",user,page);
		return hyperUser(list);
	}

	@Override
	public List<User> queryAllUsers() {
		List<User> list= this.selectList("UserMapper.queryAllUsers",null);
		return list;
	}

	@Override
	public Integer queryUserByNoNotPage(String studentNo) {
		return this.selectOne("UserMapper.queryUserIdByNo",studentNo) ;
	}

	@Override
	public User queryUserByStudentNoNotPage(String studentNo) {
		return this.selectOne("UserMapper.queryUserByStudentNoNotPage",studentNo) ;
	}
	@Override
	public User queryUserByIdAndClassId(String studentNo, Integer classId) {
		User user =new User();
		user.setClassId(classId);
		user.setNo(studentNo);
		return  this.selectOne("UserMapper.queryUserByIdAndClassId",user);
	}

	List<HyberUser>  hyperUser(List<User> list){
		List<HyberUser> list1=new ArrayList<>();
		if(list!=null) {

			for (User user : list) {
				HyberUser hyberUser = new HyberUser();
				hyberUser.setUserId(user.getUserId());
				hyberUser.setClassId(user.getClassId());
				if (ObjectUtils.isNotNull(stuClassService.getStuClassById(user.getClassId()))) {
					hyberUser.setClassname(stuClassService.getStuClassById(user.getClassId()).getClassName());
				}
				hyberUser.setEmail(user.getEmail());
				hyberUser.setMobile(user.getMobile());
				hyberUser.setAge(user.getAge());
				String name = user.getUserName();
				hyberUser.setUserName(name);
				hyberUser.setIdCard(user.getIdCard());
				hyberUser.setNo(user.getNo());
				hyberUser.setIsavalible(user.getIsavalible());
				list1.add(hyberUser);
			}
		}
			return list1;
	...
	@Override
	public List<HyberUser> queryUserByClassId(int id,PageEntity page) {
        User user =new User();
		user.setClassId(id);
		List<User> list1= this.queryForListPage("UserMapper.queryUserByClassId",user,page);
		return hyperUser(list1);
	}
