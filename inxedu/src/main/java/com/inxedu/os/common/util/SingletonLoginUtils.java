...
public class SingletonLoginUtils {
	...
	public static int getLoginTeaUserId(HttpServletRequest request) {
		Teacher useObject = getLoginTeacher(request);
		if (ObjectUtils.isNotNull(useObject)) {
	...
	public static Teacher getLoginTeacher(HttpServletRequest request){
		String userKey = WebUtils.getCookie(request,CacheConstans.LOGIN_MEMCACHE_TEACHER);
		if(StringUtils.isNotEmpty(userKey)){
			Teacher teacher = (Teacher) EHCacheUtil.get(userKey);
			//User user = (User) request.getSession().getAttribute(userKey);
