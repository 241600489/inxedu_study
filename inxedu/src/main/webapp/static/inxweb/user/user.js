...
function select(id) {
	$.get("/uc/getCourseLabByCourseTheoryIdByView/"+id,null,function (info) {
		if(confirm("你确定要选择:"+info.message)) {

					location="/uc/selectCourse/"+id;
...
/*function se(id) {
	location="/uc/initUpdateUser/0/"+id;
}*/
...
function updateUserInfo(userId){
	...
	if(emailVal==''){//验证邮箱是否为空
		alert("请输入邮箱!")
		$("#u_email").next().html('<span>请输入邮箱！</span>');
		return;
	}
	var reg=/^([a-zA-Z0-9_.-])+@([a-zA-Z0-9_])+(.[a-zA-Z0-9_])+/; //验证邮箱正则
	if(reg.test(emailVal)==false){//格式不正确
		alert("请输入正确的邮箱！");
		$("#u_email").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>请输入正确的邮箱！</span>');
		return;
	};*/
	var mobileVal=$("#u_mobile").val();
	if(mobileVal==""){//验证手机是否为空
		alert("请输入用户手机号！");
		$("#u_mobile").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>请输入用户手机号！</span>');
		return;
	}
	var reg=/^(13[0-9]|15[012356789]|18[012356789]|14[57]|17[012356789])[0-9]{8}$/; //验证手机正则
	if(reg.test(mobileVal)==false){//格式不正确
		alert("请输入正确的手机！");
		$("#u_mobile").next().html('<span class="c-orange"><em class="icon16 u-a-cw">&nbsp;</em>请输入正确的手机！</span>');
		return;
	};
	var params='';
