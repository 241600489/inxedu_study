...
<div id="aCoursesList" class="bg-fa of">
	...
	<section class="container">
		<section class="path-wrap txtOf hLh30">
			...
			\ <span class="c-333 fsize14">${course.courseName}
		</span>
		</section>
		<div>
			...
			<aside class="c-attr-wrap">
				<section class="ml20 mr15">
					<h2 class="hLh30 txtOf mt151">
						<span class="c-fff fsize24">${course.courseName}
						<c:if test="${course.type=='false'}">
								(实验课)
							</c:if>
						</span>
					</h2>
					<section class="c-attr-jg1">
						<%--<span class="c-fff">价格：</span><big class="c-yellow">￥${course.currentPrice }</big>--%>
						<%--<span class="c-ccc ml10">原价：<s>￥${course.sourcePrice }</s></span>--%>
					</section>
					<section class="c-attr-mt c-attr-undis">
						<span class="c-fff fsize14">上课教室：${course.classroom}</span><br>
							<span class="c-fff fsize14">上课时间：
						<c:if test="${course.week1!=null}">
							<br>星期一 &nbsp; 第${course.week1}大节&nbsp;(${course.weekIds})
						</c:if>
						<c:if test="${course.week2!=null}">
							<br>星期二  &nbsp;第${course.week2}大节&nbsp;(${course.weekIds})
						</c:if>
						<c:if test="${course.week3!=null}">
							<br>星期三  &nbsp;第${course.week3}大节&nbsp;(${course.weekIds})
						</c:if>
						<c:if test="${course.week4!=null}">
							<br>星期四  &nbsp;第${course.week4}大节&nbsp;(${course.weekIds})
						</c:if>
						<c:if test="${course.week5!=null}">
							<br>星期五 &nbsp;第${course.week5}大节&nbsp;(${course.weekIds})
						</c:if>
						<c:if test="${course.week6!=null}">
							<br>星期六  &nbsp;第${course.week6}大节&nbsp;(${course.weekIds})
						</c:if>
					...
					<section class="c-attr-mt">
						<a href="javascript:void(0)" title="我要选修" onclick="if(isLogin()){ window.location.href='/uc/index'} else{lrFun();} " class="comm-btn1 c-btn-3">我要选修<small>(${course.maxNumber-course.residaulCount}/${course.maxNumber})</small></a>
			...
			<aside class="thr-attr-box">
				<ol class="thr-attr-ol clearfix">
					<li><p>&nbsp;</p>
						<aside>
							<span class="c-fff f-fM">课程容量</span><br>
							<h6 class="c-fff f-fM mt10">${course.maxNumber}</h6>
						</aside></li>
					<li><p>&nbsp;</p>
						<aside>
											...
											<ul>
												...
												<c:forEach items="${parentKpointList }" var="parentKpoint" varStatus="index">
													<c:if test="${parentKpoint.kpointType==0 }"><!-- 文件目录 -->
														...
														>
																	<span class="fr">
																		<em class="icon14 m-tree-icon">&nbsp;</em></span>
															<em class="lh-menu-i-1 icon24 mr5"><font>${folderIndex }</font></em>${parentKpoint.name }</a>
														<ol class="lh-menu-ol"
					...
					<%--<div>
						...
						<section class="stud-act-list">
							<c:forEach items="${teacherList}" var="tea">
								...
								&lt;%&ndash; <div class="c-teacher-txt-show">
                                    <p>${tea.career }</p>
                                </div> &ndash;%&gt;
							</c:forEach>
						</section>
								...
								<ol>
									<c:forEach items="${courseList}" var="interfixCourse">
										<li>
											...
											<section class="hLh20 txtOf">
												<span class="c-master">${interfixCourse.maxNumber-interfixCourse.residaulCount}人</span>
											</section>
										</li>
									</c:forEach>
